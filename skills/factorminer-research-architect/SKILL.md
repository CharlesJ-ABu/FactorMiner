...
2. 先复用已有输入流、算子、Fitness Hook 和 Miner。
3. 按最小扩展原则路由：
   - 已有表达能力足够：只生成表达式或配置；
...
4. 默认只在 `user_workspace/` 下创建或修改用户资产。除非用户明确要求，不修改 `core/`、`api/` 或 `web/`。
5. 生成最小配置、测试或验证步骤；修改代码后运行与风险相称的检查。
6. 只有获得真实数据和成功运行证据后，才能报告实际指标。
7. 使用 Inspector 在明确的标的和样本外时期复查候选，并区分挖掘评分与审查结果。
